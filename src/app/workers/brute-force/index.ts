...
ctx.addEventListener("message", message => {
  ...
  if (messageData.type === Actions.SearchStart) {
    ...
    if (searchWorkersWrapper.progress)
      throw new Error(
        "Cannot start searching as the search is already in progress."
      );
    ...
    if (typeof actionMessage.maxThreadCount !== "number")
      throw new Error(
        "Cannot start searching as max count of threads is not specified."
      );

    if (actionMessage.maxThreadCount < 1)
      throw new Error(
        "Cannot start searching as max count of threads is less than one."
      );
  ...
  if (messageData.type === Actions.SearchStop) {
    if (!searchWorkersWrapper.progress)
      throw new Error(
        "Cannot stop searching as the search has not been started yet."
      );
