...
export class SearchWorkersWrapper {
   ...
   */
  static generateStartSearchMessage(
    tag: string,
    board: IBoard
  ): IStartSearchMessage {
  ...
  setKnight(knight: Knight) {
    if (!knight) throw new Error("Knight is not specified.");
  ...
  start(maxThreadCount: number) {
    if (!this._knight) throw new Error("Knight is not specified.");
    ...
    this._subs.push(
      this._notificationsHandler.generalNotification.subscribe((message) =>
        this._notificationSubject.next(message)
      )
    );

    this._subs.push(
      this._notificationsHandler.searchStopNotification.subscribe((message) =>
        this.searchStopHandler(message)
      )
    );
    ...
    for (let i = 0; i < maxThreadCount && i < this._boards.length; i++) {
      ...
      searchWorker.addEventListener("message", (ev) =>
        this._notificationsHandler.handle(ev.data)
      );
      this._searchWorkers.push(searchWorker);
    }

    const boardsToStartFrom = this._boards.splice(
      0,
      this._searchWorkers.length
    );

    this._searchWorkers.forEach((worker, index) => {
      const startMessage = SearchWorkersWrapper.generateStartSearchMessage(
        `${index + 1}`,
        boardsToStartFrom[index].asJSON()
      );
  ...
  /**
   * tags of the workers which have nothing to do
   */
  private _stoppedSeachWorkerTags: string[];
  ...
  private getBoardsToSolve(minCount: number): Board[] {
    ...
    do {
      ...
      if (boards.length === maxBoardsFound) break;
  ...
  private searchStopHandler(message: ISearchStoppedMessage) {
    ...
    if (boards.length === 0) {
      this._stoppedSeachWorkerTags.push(message.tag);

      if (this._searchWorkers.length === this._stoppedSeachWorkerTags.length) {
        this.stop();
      }
    ...
    if (boards.length > 0) {
      ...
      if (board) {
        const searchWorkerIndex = +message.tag - 1;
        const startMessage = SearchWorkersWrapper.generateStartSearchMessage(
          `${searchWorkerIndex + 1}`,
          board.asJSON()
        );
        searchWorkers[searchWorkerIndex].postMessage(startMessage);
      }
    }
  }
